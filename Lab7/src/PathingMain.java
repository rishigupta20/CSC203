...
public class PathingMain extends PApplet
{
   ...
   private boolean dfs(Point pos, GridValues[][] grid, List<Point> path) {
      ...
      if (withinBounds(pos, grid) && grid[pos.y][pos.x] == GridValues.GOAL) {
         ...
         if (withinBounds(pos, grid) && grid[pos.y][pos.x] != GridValues.SEARCHED && grid[pos.y][pos.x] != GridValues.OBSTACLE) {
            ...
            p = dfs(right, grid, path) || dfs(down, grid, path) || dfs(left, grid, path) || dfs(up, grid, path);
            if (p) {
               path.add(pos);
               return true;
         }
